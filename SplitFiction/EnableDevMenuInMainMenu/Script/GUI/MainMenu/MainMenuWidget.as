...
class UMainMenuWidget : UMainMenuStateWidget
{
	...
	void OnTransitionEnter(EMainMenuState PreviousState, bool bSnap) override
	{
		...
		if (!Debug::IsUXTestBuild())
			...
			if (Game::HazeGameInstance != nullptr)
			{
				...
				else if (CVar_EnableDevMenu.GetInt() != 0)
					DevMenuButton.SetVisibility(ESlateVisibility::Visible);
			}
			else
			DevMenuButton.SetVisibility(ESlateVisibility::Collapsed);
	...
	private void OnPlayLocalWireless(UMenuButtonWidget Button)
	{
		if (Online::OnlinePlatformName == "Sage")
		{
			...
			HostLanOption.Label = FText::FromString("Host through Local Wireless");//NSLOCTEXT("HostLobby", "HostLobbyLan", "Host through Local Wireless");
			HostLanOption.DescriptionText = FText::FromString("Host a game through Nintendo local wireless.\n\nOnly friends playing Split Fiction on Nintendo Switch 2 can join the game.");//NSLOCTEXT("HostLobby", "HostLobbyNintendo_Description", "Host a game through Nintendo local wireless.\n\nOnly friends playing Split Fiction on Nintendo Switch 2 can join the game.");
			...
			FindJoinLanOption.Label = FText::FromString("Find and Join through Local Wireless");//NSLOCTEXT("HostLobby", "FindLobbyLan", "Find and Join through Local Wireless");
			FindJoinLanOption.DescriptionText = FText::FromString("Find and Join a game through Nintendo local wireless.");//NSLOCTEXT("HostLobby", "FindJoinLobbyNintendo_Description", "Find and Join a game through Nintendo local wireless.");

			FMessageDialogOption PolestarOption;
			PolestarOption.OnChosen = FOnMessageDialogOptionChosen(this, n"OnHostPolestar");
			PolestarOption.Label = FText::FromString("Host a game using Polestar");
			PolestarOption.DescriptionText = FText::FromString("Host a game using Polestar");
